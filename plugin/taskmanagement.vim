...
if !exists('g:loaded_taskmanagement') || &cp
    ...
    if !exists('$VIMTASKMANAGEMENTDIR')
    	let $VIMTASKMANAGEMENTDIR = '~/Documents/Wiki/task'
    endif
    ...
    autocmd BufWrite $VIMTASKMANAGEMENTDIR/*.tm :helptags $VIMTASKMANAGEMENTDIR
    autocmd BufRead,BufNewFile $VIMTASKMANAGEMENTDIR/*.tm :set filetype=taskmanagement
endif
...
function! Todo(day, ...)
    ...
    try
        exec "lvimgrep /=" . _date . "/j " . $VIMTASKMANAGEMENTDIR . "/*.tm"
    catch /^Vim(\a\+):E480:/
    endtry
    ...
    if a:0 > 0
        ...
        for offset in range(a:day+1, a:1)
            ...
            try
                exec "lvimgrepadd /=" . _date . "/j " . $VIMTASKMANAGEMENTDIR . "/*.tm"
            catch /^Vim(\a\+):E480:/
            endtry
        endfor
    endif
...
function! PendingTodo(days)
    ...
    try
        exec "lvimgrep /.*=" . _date . "\\(.*=done\\)\\@!.*$/j ". $VIMTASKMANAGEMENTDIR . "/*.tm"
    catch /^Vim(\a\+):E480:/
    endtry
    ...
    if a:days > 1
        for offset in range(2, a:days)
            let _date = strftime("%d%m%Y", localtime()-offset*86400)
            try
                exec "lvimgrepadd /.*=" . _date . "\\(.*=done\\)\\@!.*$/j " . $VIMTASKMANAGEMENTDIR . "/*.tm"
            catch /^Vim(\a\+):E480:/
            endtry
        endfor
    endif
